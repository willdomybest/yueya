...
一个 Python 文件，把任何一台电脑变成浏览器里的文件管理器：不装客户端、不上传云端，打开浏览器就能管理自己的文件。
...
- 不需要 FTP 可以不装 `pyftpdlib`，程序会自动跳过。
- 装了 [uv](https://docs.astral.sh/uv/) 可以一行运行：`uv run RemoteFM.py`。
- 手机或局域网设备访问 `http://本机IP:8880`，首次运行记得让防火墙放行。
...
不想为了拿一个文件先装客户端、登录、等同步，也不想把自己的文件交给别人的服务器。RemoteFM 只做一件事：**在你自己的机器上开一扇门，用浏览器管理它。**

- **只有一个文件**：后端、页面、前端全在 `RemoteFM.py` 里，没有配置文件、没有数据库、没有构建步骤，想读能读、想改能改。
- **数据不出门**：不联网上报、没有埋点，文件只在你自己和你信任的设备之间流动。
- **旧机器也跑得动**：Windows / macOS / Linux 通吃，Python 3.8 起步，树莓派、NAS、十年前的老笔记本都不挑。

常见用法：家庭私有云盘 · 无头服务器 / 树莓派 / NAS 的图形界面 · 手机上管理电脑文件 · 临时给同事传文件（发个链接或开 FTP，对方零安装）。

## 安全

**登录密码 = 这台电脑的控制权**：它能读写文件、删除文件、执行命令。

- 第一件事：改掉默认密码 `admin / admin123`；
- 只在局域网或 VPN 内用，不要直接暴露到公网；
- 用普通用户运行，并把 `SD_ROOT` 指向需要管理的目录；
- 默认是明文 HTTP，公网使用请放到 HTTPS 反向代理之后并加上访问控制。
